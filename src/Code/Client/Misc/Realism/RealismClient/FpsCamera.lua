...
function FpsCamera:OnRotationTypeChanged()
	...
	if subject and subject:IsA("Humanoid") then
		...
		if rotationType == Enum.RotationType.CameraRelative then
			subject:SetAttribute("AutoRotate", false) -- FORK: "subject.AutoRotate" makes the head float in the air if used by other scripts, changed it to an attribute instead, which I imagine will break something but it's better than the alternative.
			
			RunService:BindToRenderStep("FpsCamera", 1000, function (delta)
				if subject:GetAttribute("AutoRotate") or not subject:IsDescendantOf(game) or (subject.SeatPart and subject.SeatPart:IsA("VehicleSeat")) then
				...
				if rootPart and not isGrounded then
					...
					if canRotate and (subject:GetState() ~= Enum.HumanoidStateType.Ragdoll) then -- FORK: Ragdoll breaks this
			...
			subject:SetAttribute("AutoRotate", true)
			self.MirrorBin.Parent = nil
		end
	end
end
